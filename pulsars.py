...
class psr(object):
    def __init__(self, params, parts, indices):
        # Do RAJ and DECJ first
        posn = c.SkyCoord(parts[indices['RAJ']]+" "+parts[indices['DECJ']], frame=c.ICRS, unit=(u.hourangle, u.deg))
        for param in params:
            ...
            if param=="NAME":
                ...
                if not parts[part_index]=='*':
                    ...
                    self.p, self.perr = ro.pferrs(self.f, self.ferr)
                else:
                ...
                if not parts[part_index]=='*':
                    ...
                    self.p, self.perr, self.pd, self.pderr = ro.pferrs(self.f, self.ferr, self.fd, self.fderr)
            elif param=="F2":
                if not parts[part_index]=='*':
                    self.fdd, self.fdderr = float(parts[part_index]), float(parts[part_index+1])
                    self.p, self.pd, self.pdd = ro.p_to_f(self.f, self.fd, self.fdd)
            elif param=="F3":
                if not parts[part_index]=='*':
    ...
    def __str__(self):
        ...
        if (self.type is not None):
            out = out + "                  Type = %s\n" % self.type
        (h, m, s) = ro.rad_to_hms(self.ra)
        serr = pc.RADTOSEC * self.raerr
        out = out + "            RA (J2000) = %s +/- %.4fs\n" % \
              (ro.coord_to_string(h, m, s), serr)
        (d, m, s) = ro.rad_to_dms(self.dec)
        serr = pc.RADTOARCSEC * self.decerr
        out = out + "           DEC (J2000) = %s +/- %.4f\"\n" % \
              (ro.coord_to_string(d, m, s), serr)
        out = out + "                (l, b) = (%.2f, %.2f)\n" % \
              (self.l, self.b)
...
if __name__ == "__main__":
    ...
    outfile = os.path.splitext(fi)[0] + ".cat"
    with open( outfile, "wb" ) as out:
        print( f"Writing {len(psrs)} pulsars ({num_binaries} binaries) to {outfile}" )
        for ii, psr in enumerate(psrs):
            try:
                out.write( psr.pack_structs() )
            except:
                print( ii, psr.jname )
